...
export default function Header() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        const res = await fetch("/user/api/verify-magic-link", {
          method: "GET",
        });
        if (res.ok) {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      {/* Main Navigation */}
      <div
        className={`bg-white shadow-sm transition-all duration-300 ${isScrolled ? "shadow-md" : ""}`}
      >
        <div className="container w-full max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          >
            <Image
              src={logo}
              alt="Dinar Exchange Company Logo"
              width={200}
              height={60}
              priority
            ...
            <div className="flex items-center gap-1">
              ...
              <Image
                ...
                style={{ minWidth: 20, minHeight: 14 }}
              />
            </div>
          </Link>
