...
export const authOptions = {
    ...
    id: "admin-login",
      ...
      async authorize(credentials, req) {
        try {
          ...
          await AuditLog.logAction(admin._id, "login", "admin", admin._id, {
            metadata: {
              ipAddress:
                req?.headers?.["x-forwarded-for"] ||
                req?.connection?.remoteAddress,
  ...
  session: {
    strategy: "jwt",
    maxAge: process.env.NODE_ENV === "production" ? 4 * 60 * 60 : 8 * 60 * 60, // 4 hours in prod, 8 in dev
  ...
  events: {
    async signOut({ token }) {
      if (token?.sub) {
        try {
          await dbConnect();
          await AuditLog.logAction(token.sub, "logout", "admin", token.sub, {
