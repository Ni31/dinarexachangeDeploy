...
// ====== Main Page ======
export default function BuyZimbabweDollar() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // POST order (replace file objects with URLs)
      const payload = {
        ...formData,
        idFileUrl,
        paymentReceiptUrl,
...
function Stepper({ step }) {
  return (
    <div className="flex justify-center gap-10 lg:gap-20 mb-4 relative">
      {[
        { icon: DocumentTextIcon, label: "Order Details" },
        { icon: IdentificationIcon, label: "ID Verification" },
