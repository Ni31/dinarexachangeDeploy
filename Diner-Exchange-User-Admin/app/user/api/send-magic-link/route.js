...
export async function POST(request) {
  try {
    ...
    if (!user.magicKey) {
      user.magicKey = randomBytes(32).toString("hex");
    }
...
`;
    ...
    // Send the magic link email using Resend (only if API key is configured)
    if (process.env.RESEND_API_KEY && process.env.RESEND_API_KEY !== 'your_resend_api_key_for_testing') {
      ...
      console.log('Email service not configured - check RESEND_API_KEY');
    }
